...
export const unfollow = async (userId: string) => {
  const res = await httpRequest.post("/user/unfollow", { user: userId });
  return res;
};

export const follow = async (userId: string) => {
  const res = await httpRequest.post("/user/follow", { user: userId });
  return res;
};

// bi ooriin ywuulsan req ustgana
export const removeRequestCancel = async (userId: string) => {
...
export const likePage = async (id: string) => {
  const res = await httpRequest.post(`/user/page/${id}/like`);
  return res;
};
export const unlikePage = async (id: string) => {
